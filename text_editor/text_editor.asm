...
section .data
   ...
   starss db '****************',10,0
   user_interface_msg db '1-search a word',10,'2-search and replace',10,'3-append',10,'4-remove',10,'5-save',10,'6-save as',10,'7-editing with cursor(dose not work properly)',10,'8-end',0
   ...
   escape_msg db 'press escape 3 times for confirm:',10,0
section .bss
...
shift_left:
   ...
   dec rsi
   mov rax , [starting_character]
   add rax , r12; check end of file
   add rax , 2
   cmp rax , rdx
   je nulling_word_setter
   add rsi , r14
   dec rsi
   mov rdx , r13
   dec rdx
...
nulling_word_setter:
   ...
   mov rax , r11
   mov rbx , r12
nulling_word_handler:
   cmp rax , rbx
   jg null_ret
   mov byte[rsi],0
   inc rsi
   inc rax
   jmp nulling_word_handler
null_ret:
   jmp put_new_word_setter
put_new_word_setter:
   mov rsi , buffer
   mov rdi , replaced_string
   add rsi , [starting_character]
...
clear_terminal:
   ...
   ret
cursor_editing:
   call clear_terminal ;press ctrl+z for finishing
   mov rdi , append_msg
   mov rsi , buffer
   call printString
   mov rsi , buffer
